#ifndef __linux__
/* Low-level GUS routines by MikMak/HardCode and Jedi/Sector One */
...
/*      GF1OutB(MASTER_RESET,GF1_MASTER_RESET|GF1_OUTPUT_ENABLE); */
    GF1OutB(MASTER_RESET,
	    GF1_MASTER_RESET | GF1_OUTPUT_ENABLE | GF1_MASTER_IRQ);
}
...
BOOL UltraDetect(void)
{
    ...
    if (sscanf(ptr, "%hx,%hd,%hd,%hd,%hd",
	       &GUS_PORT,
	       &GUS_DRAM_DMA,
	       &GUS_ADC_DMA, &GUS_GF1_IRQ, &GUS_MIDI_IRQ) != 5)
	return 0;
...
static void UltraNumVoices(int voices)
{
    ...
    UltraDisableOutput();
    UltraReset(voices);
    UltraSetInterface(GUS_DRAM_DMA, GUS_ADC_DMA, GUS_GF1_IRQ,
		      GUS_MIDI_IRQ);
}
#endif
...
gf1handler(void)
{
    ...
    while ((irq_source = inportb(GF1_IRQ_STAT)) != 0) {
...
#if 0
#ifdef DJGPP
    oldhandler = MIrq_DJSetHandlerFunc(GUS_GF1_IRQ, gf1handler,
				       ((int) EOgf1handler) -
				       ((int) gf1handler));
#else
...
#ifdef DJGPP
    MIrq_DJSetHandlerAddr(GUS_GF1_IRQ, oldhandler);
#else
...
static void UltraSetVoice(ULONG location)
{
    ...
    phys_loc =
	(data & VC_DATA_TYPE) ? convert_to_16bit(location) : location;
...
static UBYTE UltraPrimeVoice(ULONG begin, ULONG start, ULONG end,
			     UBYTE mode)
{
...
static UWORD vol_rates[19] = {
    23, 24, 26, 28, 29, 31, 32, 34, 36, 37, 39, 40, 42, 44, 45, 47, 49, 50,
	52
};
...
void UltraRampLinearVolume(UWORD start_idx, UWORD end_idx, ULONG msecs,
			   UBYTE mode)
{
    UWORD start, end;
    UBYTE rate;
